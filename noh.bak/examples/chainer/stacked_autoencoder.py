...
stacked_autoencoder = noh_.Circuit(
    noh_.Wrapper(C.Autoencoder(784, units, gpu=gpu), train=[noh_.Binder(epochs=epoch, batchsize=batchsize)]),
    noh_.Wrapper(C.Autoencoder(units, units, gpu=gpu), train=[noh_.Binder(epochs=epoch, batchsize=batchsize)]),
    noh_.Wrapper(C.Perceptron(units, 10, gpu=gpu), train=['labels', noh_.Binder(epochs=epoch, batchsize=batchsize)]),
)
